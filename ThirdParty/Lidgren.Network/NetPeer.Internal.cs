...
#else
        private System.Threading.Tasks.Task m_networkTask;
#endif
...
#elif NETFX_CORE
                Windows.Storage.Streams.IBuffer data = Windows.Security.Cryptography.CryptographicBuffer.CreateFromByteArray(combined);
                var p = Windows.Security.Cryptography.Core.HashAlgorithmProvider.OpenAlgorithm(Windows.Security.Cryptography.Core.HashAlgorithmNames.Sha1);
                var result = p.HashData(data);
                byte[] hash = new byte[result.Length];
                Windows.Security.Cryptography.CryptographicBuffer.CopyToByteArray(result, out hash);
                m_uniqueIdentifier = BitConverter.ToInt64(hash, 0);
#else
